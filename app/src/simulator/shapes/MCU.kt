...
class MCU(private val name: String,
          val width: Int,
          private val schema: CircuitComponent,
          private val pos: Point
) {

  private val pinNames = arrayOf("C6", "D0", "D1", "D2", "D3", "D4", "VCC", "GND1", "B6", "B7", "D5", "D6",
    "D7", "B0", "B1", "B2", "B3", "B4", "B5", "AVCC", "Aref", "GND2", "C0", "C1", "C2", "C3", "C4", "C5")
  ...
  fun drawTo(gr2: Graphics2D) {
    ...
    for (i in 0 until pinNames.size / 2) {
      ...
      schema.addPin(pinNames[i], Pin(Point(pos.x - pinDiam / 2, y), pinDiam), this.name)
    ...
    for (i in pinNames.size / 2 - 1 downTo  0) {
      val y = pos.y + (i + 1) * dy - dy / 2
      schema.addPin(pinNames[pinNames.size - 1 - i], Pin(Point(x, y), pinDiam), this.name)
